...
T = 0.5;
Strike = 95;
NumOfTimeSteps=8; %Elapsed time is 732.024109 seconds. for 11
 Sigma=0.10;
 lambda=sqrt(1.5);
...
[x1,delta1, gamma1 ] = AuxLatticeGreeksAsiang_31(Underlying, Strike, r, T, ...
                NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);
%x          
%delta
%gamma
[x2,delta2,gamma2]= AuxLatticeGreeksAsiang_31(min(x1), Strike, r, T, ...
                NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);
...
[x3,delta3,gamma3]= AuxLatticeGreeksAsiang_31(max(x1), Strike, r, T, ...
                NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);  
            k=0;
            for i=1:(2*NumOfTimeSteps+1)-4
            if (x3(i)>max(x1))
...
fileID = fopen('1AsianGaGreeks.txt','w');
