function [ delta ] = computeDeltaAmerican1(Underlying, Strike, r, T, ...
...
b=priceOptionTrinCEVAmericanTry1(slat{3}(1), Strike, r, T,NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);%(slat{3}(1)-S0)
c=priceOptionTrinCEVAmericanTry1(slat{3}(length(slat{3}(:))), Strike, r, T,NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);%(S0-slat{3}(length(slat{3}(:))))
