function [OptionPrice, TrinLatticeUnderlying, TrinLatticeOption] = ...
                priceOptionTrinCEVBermudanTry1(Underlying, Strike, r, T, ...
...
% Sigma: volatility
% m: dm is the Length of excerise interval for burmudan option
...
dt = T / NumOfTimeSteps;
DISFACTOR = exp(r * dt);
dm=fix(NumOfTimeSteps/M_Bermudan);
%------------------------------
...
for Idx = NumOfTimeSteps:-1:1
    for k = 1:length(TrinLatticeOption{Idx})
        ...
        %------------------ 
        if rem(k,dm)==0 %If the period is an excercise period the apply American Payoff
     TrinLatticeOption{Idx}(k) = max(calcPayoff(TrinLatticeStock{Idx+1}(k),Strike, StrCallPut),...
        ...
        else %Otherwise apply European payoff
        TrinLatticeOption{Idx}(k) = ...
           DISFACTOR^-1 * (ProbLattice{Idx+1}(k+2*(k-1)+0) * TrinLatticeOption{Idx+1}(k+2*(k-1)+0) ...
                + ProbLattice{Idx+1}(k+2*(k-1)+1) * TrinLatticeOption{Idx+1}(k+2*(k-1)+1) ... 
                + ProbLattice{Idx+1}(k+2*(k-1)+2) * TrinLatticeOption{Idx+1}(k+2*(k-1)+2));
