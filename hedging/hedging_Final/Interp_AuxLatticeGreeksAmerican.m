function [ greeks] = Interp_AuxLatticeGreeksAmerican(Underlying, Strike, r, T, ...
                NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut, Strdeltagamma)


            
            
[x1,delta1, gamma1 ] = AuxLatticeGreeksAmerican(Underlying, Strike, r, T, ...
                NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);

[x2,delta2,gamma2]= AuxLatticeGreeksAmerican(min(x1), Strike, r, T, ...
                NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);
            k=0;
...
[x3,delta3,gamma3]= AuxLatticeGreeksAmerican(max(x1), Strike, r, T, ...
                NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);  
            k=0;
            for i=1:(2*NumOfTimeSteps+1)-4
            if (x3(i)>max(x1))
...
delta = interp1(newXSamplePoints,smoothedYDelta,Underlying,'pchip');
 
gamma = interp1(newXSamplePoints, smoothedYGamma, Underlying,'pchip');
