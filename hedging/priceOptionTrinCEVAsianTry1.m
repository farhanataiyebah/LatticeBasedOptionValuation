function [OptionPrice, TrinLatticeUnderlying, TrinLatticeOption] = ...
                priceOptionTrinCEVAsianTry1(Underlying, Strike, r, T, ...
...
TrinLatticeStockSum = buildLatticeAsian(NumOfTimeSteps+1, 'trin');
TrinLatticeStockMean = buildLatticeAsian(NumOfTimeSteps+1, 'trin');
%--------------------------
TrinLatticeUnderlying{1}(1) = Underlying;
TrinLatticeStockSum{1}(1) = Underlying;
TrinLatticeStockMean{1}(1) = Underlying;
%--------------------------
for Idx = 2:NumOfTimeSteps+1
    ...
    %---------------------------
    TrinLatticeStockSum{Idx}(Onelag+2*(Onelag-1)+0) =sum([TrinLatticeStockSum{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+0)]);
    TrinLatticeStockSum{Idx}(Onelag+2*(Onelag-1)+1) =sum([TrinLatticeStockSum{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+1)]);
    TrinLatticeStockSum{Idx}(Onelag+2*(Onelag-1)+2) =sum([TrinLatticeStockSum{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+2)]);
    %---------------------------
    TrinLatticeStockMean{Idx}(Onelag+2*(Onelag-1)+0) =sum([TrinLatticeStockSum{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+0)])/Idx;
    TrinLatticeStockMean{Idx}(Onelag+2*(Onelag-1)+1) =sum([TrinLatticeStockSum{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+1)])/Idx;
    TrinLatticeStockMean{Idx}(Onelag+2*(Onelag-1)+2) =sum([TrinLatticeStockSum{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+2)])/Idx;
...
for k = 1:length(TrinLatticeOption{end})
    TrinLatticeOption{end}(k) = calcPayoff(TrinLatticeStockMean{end}(k), ...
        Strike, StrCallPut);
end
