function [ x,delta, gamma ] = AuxLatticeGreeksAsiang_31(Underlying, Strike, r, T, ...
...
[price, slat, plat] = priceOptionTrinCEVAsianGTry1(S0, Strike, r, T, ...
 ...
 k=0;
            for j=NumOfTimeSteps+1:-1:1 %upper nodes
                k=k+1;
                s(k)=slat{j}(1);
                p(k)=priceOptionTrinCEVAsianGTry1(slat{j}(1), Strike, r, T, ...
               NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);
           ...
           for j=2:NumOfTimeSteps+1 %lower nodes
               k=k+1;
                ...
                p(k)=priceOptionTrinCEVAsianGTry1(slat{j}(length(slat{j}(:))), Strike, r, T, ...
               NumOfTimeSteps, Sigma,alpha,lambda,M, StrCallPut);
