function [Delta_approx, Del_Gam_approx,FullValue_approx ] = AsianGa_Approximations1(S0,St,K1,K3,m1,m2,m3,T,tau,Rate,Sigma,NumOfTimeSteps,alpha,lambda,M)

%put = Call_1 + K1*exp(-Rate*T)- S0*exp(-Rate*T);
Call_1=priceOptionTrinCEVAsianGTry1(S0, K1, Rate,T, ...
               NumOfTimeSteps,  Sigma,alpha,lambda,M, 'Call');
put=priceOptionTrinCEVAsianGTry1(S0, K1, Rate,T, ...
               NumOfTimeSteps,  Sigma,alpha,lambda,M, 'Put');
            
Call_2=priceOptionTrinCEVAsianGTry1(S0, K3, Rate,T, ...
              NumOfTimeSteps,  Sigma,alpha,lambda,M, 'Call');
 %%
...
Del_prfl    = m1*(interpolationGreeks_K1(S0,'pchip', 'Put','Ag', 'delta')) +  m2*(interpolationGreeks_K1(S0,'pchip', 'Call','Ag', 'delta'))...,  
                    + m3* (interpolationGreeks_K3(S0,'pchip', 'Call','Ag', 'delta'));
                
Gam_prfl    = m1*(interpolationGreeks_K1(S0,'pchip', 'Put','Ag', 'gamma')) +  m2*(interpolationGreeks_K1(S0,'pchip', 'Call','Ag', 'gamma'))...,  
                    + m3* (interpolationGreeks_K3(S0,'pchip', 'Call','Ag', 'gamma'));
                 
%[BS_Call_1, BS_Put] = blsprice(St, K1, r_51, T_51-tau, est_sig, q_51);
%BS_Call_2 = blsprice(St, K3, r_51, T_51-tau ,est_sig, q_51);

Asian_Call_1=priceOptionTrinCEVAsianGTry1(St, K1, Rate, T-tau, ...
                NumOfTimeSteps,  Sigma,alpha,lambda,M, 'Call');
Asian_Put=priceOptionTrinCEVAsianGTry1(St, K1, Rate, T-tau, ...
                NumOfTimeSteps,  Sigma,alpha,lambda,M, 'Put');
Asian_Call_2 =priceOptionTrinCEVAsianGTry1(St, K3, Rate, T-tau, ...
                NumOfTimeSteps,  Sigma,alpha,lambda,M, 'Call');
