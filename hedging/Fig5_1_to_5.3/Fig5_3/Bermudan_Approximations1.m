function [Delta_approx, Del_Gam_approx,FullValue_approx ] = Bermudan_Approximations1(S0,St,K1,K3,m1,m2,m3,T,tau,Rate,Sigma,NumOfTimeSteps, M_Bermudan,alpha,lambda,M)
...
Del_prfl    = m1*(interpolationGreeks_K1(S0,'pchip', 'Put','B', 'delta')) +  m2*(interpolationGreeks_K1(S0,'pchip', 'Call','B', 'delta'))...,  
                    + m3* (interpolationGreeks_K3(S0,'pchip', 'Call','B', 'delta'));
                
Gam_prfl    = m1*(interpolationGreeks_K1(S0,'pchip', 'Put','B', 'gamma')) +  m2*(interpolationGreeks_K1(S0,'pchip', 'Call','B', 'gamma'))...,  
                    + m3* (interpolationGreeks_K3(S0,'pchip', 'Call','B', 'gamma'));
                 
%[BS_Call_1, BS_Put] = blsprice(St, K1, r_51, T_51-tau, est_sig, q_51);
%BS_Call_2 = blsprice(St, K3, r_51, T_51-tau ,est_sig, q_51);

Bermudan_Call_1=priceOptionTrinCEVBermudanTry1(St, K1, Rate, T-tau, ...
                NumOfTimeSteps, M_Bermudan,  Sigma,alpha,lambda,M, 'Call');
Bermudan_Put=priceOptionTrinCEVBermudanTry1(St, K1, Rate, T-tau, ...
                NumOfTimeSteps,  M_Bermudan, Sigma,alpha,lambda,M, 'Put');
Bermudan_Call_2 =priceOptionTrinCEVBermudanTry1(St, K3, Rate, T-tau, ...
                NumOfTimeSteps, M_Bermudan,  Sigma,alpha,lambda,M, 'Call');
