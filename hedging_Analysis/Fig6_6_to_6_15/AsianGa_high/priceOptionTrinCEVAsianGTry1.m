function [OptionPrice, TrinLatticeUnderlying, TrinLatticeOption] = ...
                priceOptionTrinCEVAsianGTry1(Underlying, Strike, r, T, ...
...
TrinLatticeStockProd = buildLatticeAsian(NumOfTimeSteps+1, 'trin');
TrinLatticeStockGMean = buildLatticeAsian(NumOfTimeSteps+1, 'trin');
%--------------------------
TrinLatticeUnderlying{1}(1) = Underlying;
TrinLatticeStockProd{1}(1) = Underlying;
TrinLatticeStockGMean{1}(1) = Underlying;
%--------------------------
for Idx = 2:NumOfTimeSteps+1
    ...
    %---------------------------
    TrinLatticeStockProd{Idx}(Onelag+2*(Onelag-1)+0) =prod([TrinLatticeStockProd{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+0)]);
    TrinLatticeStockProd{Idx}(Onelag+2*(Onelag-1)+1) =prod([TrinLatticeStockProd{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+1)]);
    TrinLatticeStockProd{Idx}(Onelag+2*(Onelag-1)+2) =prod([TrinLatticeStockProd{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+2)]);
    %---------------------------
    TrinLatticeStockGMean{Idx}(Onelag+2*(Onelag-1)+0) =prod([TrinLatticeStockProd{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+0)])^(1/Idx);
    TrinLatticeStockGMean{Idx}(Onelag+2*(Onelag-1)+1) =prod([TrinLatticeStockProd{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+1)])^(1/Idx);
    TrinLatticeStockGMean{Idx}(Onelag+2*(Onelag-1)+2) =prod([TrinLatticeStockProd{Idx-1}(Onelag) TrinLatticeUnderlying{Idx}(Onelag+2*(Onelag-1)+2)])^(1/Idx);
...
for k = 1:length(TrinLatticeOption{end})
    TrinLatticeOption{end}(k) = calcPayoff(TrinLatticeStockGMean{end}(k), ...
        Strike, StrCallPut);
end
